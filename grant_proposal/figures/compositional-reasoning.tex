...
\node[below=1.5cm of g1, text width=10cm, align=center, font=\small\sffamily] (eq) {
...
\node[above=0.8cm of gin, text width=10cm, align=center, font=\small\sffamily\itshape, color=black!50] {
  ProgramEncoder infers composition from $K{=}3$ support pairs (input $\to$ output examples)
};

% ============ BRACE: "Learned from support" ============
\draw[decorate, decoration={brace, amplitude=5pt, raise=3pt}, thick, color=arrowcol]
  (gin.north west) -- (gout.north east)
  node[midway, above=10pt, font=\scriptsize\sffamily, color=arrowcol] {Inferred program: (rot90, 1) $\to$ (col\_inc, 1)};
