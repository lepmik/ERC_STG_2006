...
\begin{itemize}[noitemsep,topsep=2pt]
  \item RQ1: How can memory-augmented modules distribute and combine long-term temporal information?
  \item RQ2: How can such modules support active inference and causal reasoning?
  \item RQ3: How can neurosymbolic components be integrated into distributed memory architectures without sacrificing gradient-based learning?
  \item RQ4: Does this integration measurably enhance compositional reasoning in complex environments?
  \item RQ5: Can such networks learn adaptively and reason about causal relations over long time horizons?
\end{itemize}

\subsection{Research methodology}

One prediction-error objective governs PAM's three operations on a shared memory---encode, retrieve, and act---made concrete by the update equations below. Each module $m$ has latent state $z^m_t \in \mathbb{R}^{d_z}$ and sensory input $x^m_t \in \mathbb{R}^{d_x}$ at time $t$. The prediction error $e_t = x^m_t - \hat{x}^m_t$ drives module state updates: $z^m_{t+1} = f_m(z^m_t, c^m_t, r^m_t, a_t)$, where $f_m$ is a learnable function, $a_t$ is the action, $r^m_c = \sum_i \alpha^m_c[i]h^m_i$ is context-dependent memory retrieval, and $c^m_t$ is contextual input from inter-module communication. Gradients flow end-to-end through Gumbel-softmax routing and soft attention over memory slots on the read path; discrete WTA writes are handled with straight-through estimators (soft-read/hard-write), so encoder and modules train jointly while writes remain discrete. Beyond these per-module operations, modules must exchange information; two orchestration mechanisms will be explored:
...
\subsubsection*{WP1: Temporal Predictive Coding with Distributed Memory \textnormal{(Lepperød, PhD1; RQ1; Years 1--3)}}

WP1 establishes the encode and retrieve routes---temporal prediction into memory and content-addressable recall out of it. Building on our preliminary results ($98.1\%$ exact recall at $\sigma{=}0.4$, $3.75{\times}$ less forgetting than tPC-alone), WP1 extends the architecture to challenging temporal benchmarks and characterizes its operational regime.

\noindent\textbf{Benchmark strategy.} We adopt a four-tier evaluation hierarchy (Figure~\ref{fig:tiers}) designed to systematically validate PAM's capabilities and position it against the state of the art:
\begin{itemize}[noitemsep,topsep=2pt]
...
\textbf{Completed:} associative-memory substrate (94\% exact recall at 32 pairs) and gradient-free online continual learning on CORe50-NI/CLEAR-10/CIFAR-100 with frozen features (see Part~I, Fig.~\ref{fig:prelim}); the from-scratch representation regime is the open WP1 target.
...
Systematically characterise the recall--noise--capacity landscape across $\sigma \in [0.0, 0.8]$ and $K \in [10, 500]$, including an analytical derivation of the capacity cliff seen in the substrate (exact recall $0.94{\to}0.39$ as $K$ grows $32{\to}96$; Part~I) from HDC crosstalk statistics---crosstalk in superposed hypervectors grows with stored items, predicting a capacity/SNR phase transition that neither predictive coding nor HDC theory predicts alone---a falsifiable, architecture-specific prediction of PAM. Implement adaptive inference depth via prediction-error thresholding: at each iteration, compare the current prediction error against a learned threshold; terminate early when the error is below threshold, saving computation. Characterise interactions between slot count, HDC dimension, and forgetting rate under varying concept-drift speeds, providing design guidelines for WP2--3. Baselines include a memory-budget-matched episodic kNN/replay buffer---the store-everything alternative that also forgets nothing---to isolate PAM's distinctive contribution: \textit{selectivity} of what to store and retrieve under bounded memory.
\textit{Success criterion:} Adaptive inference must achieve ${\geq}95\%$ of full-iteration accuracy at ${\leq}60\%$ of compute on the MNIST benchmark, and PAM must match or exceed the budget-matched kNN baseline at equal memory footprint.

\paragraph{T1.3: Hierarchical Temporal Compression for Long-Horizon Memory}
Extend PAM with hierarchical time-scale factorisation: fast modules ($\Delta t{=}1$) process step-level inputs while slow modules compress episodes into semantic summaries via learned gating inspired by complementary learning systems~\cite{ramapuram2021kanerva}. The gating criterion---whether and when prediction error triggers consolidation from episodic to semantic memory---will be resolved experimentally using prediction-error statistics from T1.2. T1.3 also develops \textit{key reconsolidation}: periodically re-encoding stored memory keys with the current encoder so that content-addressable retrieval survives representation drift when the encoder learns online (the mechanism targeting the from-scratch representation collapse identified in Part~I, where an online-learned encoder degrades content-addressable retrieval). Evaluate on Tier~3 benchmarks: ``infinite dSprites'' ($T{=}2{,}000$--$50{,}000$) and Memory Maze ($T{=}2{,}000$--$10{,}000$)~\cite{pasukonis2022evaluating}. The key test is \textbf{Path-X} ($T{=}16{,}384$): only S4, Mamba, and a few others solve this benchmark; success here would demonstrate that HDC memory with iterative inference can match purpose-engineered SSMs on ultra-long-range tasks.

\subsubsection*{WP2: Causal Reasoning and Compositional Understanding \textnormal{(Lepperød, PD; RQ2--RQ4; Years 2--4)}}

WP2 builds out the two routes WP1 leaves unexercised: \textit{compositional retrieval}---subspace reasoning over the HDC memory---and \textit{action}, where active inference turns the same prediction-error objective into causal world-model learning. Neurosymbolic components provide explicit reasoning alongside statistical learning, and the agent discovers causal structure through interventions.

\paragraph{T2.1: Action-Augmented Temporal Predictive Coding}
T2.1 implements the \textit{act} route: action becomes a second way to minimise prediction error, complementing perceptual inference. We extend PAM with action nodes whose selection is scored by how well the resulting input is predicted to match memory---the agent acts to fulfil memory-derived predictions. Concretely, we develop action-conditioned memory addressing, action-dependent update rules across multiple timescales, and joint optimisation of state predictions and action selection while maintaining temporal consistency.
...
Develop mechanisms for compositional, subspace-based reasoning within the PAM architecture. The core idea is to factorize latent spaces into semantically meaningful subspaces that can be \textit{composed algebraically} through HDC binding operations. Concretely, this means representing transformations as operations that compose exactly: a rotation followed by a colour change should produce the same result whether applied individually or as a single bound operation. Our preliminary PAMNCA results (40\% exact match on ARC leave-one-out, 26\% on the held-out split, 1.47M parameters) validate the memory-augmented approach, and memory ablations attribute 30--43\% of performance to the distributed memory. Importantly, LoRA program decomposition experiments demonstrate that ARC programs are \textit{not} low-rank composable, motivating the shift to HDC algebraic composition which operates in $d{=}1000{+}$ dimensional space with guaranteed compositionality. We will implement: (i)~a lossless per-cell circular embedding for spatial structures; (ii)~algebraically exact primitives (rotation, reflection, translation, colour transformations) that compose via HDC binding; (iii)~a meta-learning pipeline (FOMAML~\cite{finn2017maml}) for few-shot program inference from support examples; and (iv)~HDC program storage enabling compositional search over the space of transformations. We evaluate compositional reasoning on abstract visual reasoning benchmarks including ARC~\cite{chollet2019arc}, letter-string analogies~\cite{mitchell1993analogy}, and bAbI multi-hop QA~\cite{weston2015babi}.
\textit{Success criterion:} Building on the current 40\% (ARC-AGI-1 leave-one-out) / 26\% (held-out), achieve competitive exact-match on \textbf{ARC-AGI-2} through iterative HDC composition, with interactive \textbf{ARC-AGI-3} as a stretch target. The open problem is compositional \textit{inference}: a one-shot HDC program encoder currently reaches only 29\% on held-out 2-step compositions (bar 80\%), so T2.2 develops iterative infer-apply-residual (resonator-style) encoding. Figure~\ref{fig:comp} illustrates the approach.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth, clip]{figures/compositional-reasoning.pdf}
  \caption{\textbf{Compositional reasoning through algebraic operations in HDC space.} A visual transformation (rotation + colour increment) is decomposed into composable primitives. The ProgramEncoder infers the composition from support examples. Algebraic composition guarantees out-of-distribution generalisation to novel rules without retraining (bottom table; miniARC grids, 4 held-out rules).}\label{fig:comp}
\end{figure}

\paragraph{T2.3: Causal World Model Learning through Active Inference}
Implement active inference for action selection balancing information gain and goal-oriented reward~\cite{friston2010free,rao2023active}. Expected free energy is made tractable by amortization: a learned policy posterior scores candidate sub-goals represented as HDC goal vectors, so planning operates in symbol space over bound sub-goal compositions rather than exhaustive state-space rollouts. Develop causal discovery mechanisms through interventionist exploration: the agent executes targeted interventions (changing one variable while holding others fixed), observes the downstream effects, and updates its world model to encode directional causal relationships rather than mere correlations. This follows the ``interventionist'' approach to causal learning~\cite{scholkopf2021causal}: modules learn not just that ``opening a chest produces items'' but that ``the chest \textit{caused} items to appear,'' enabling counterfactual reasoning (``what if I hadn't opened the chest?''). Build on our work with NCA~\cite{kvalsund2026sensor} and proximal policy optimization~\cite{schulman2017ppo} for credit assignment.
...
\textit{Success criterion:} PAM must achieve ${>}10$ percentage point improvement over a parameter-matched LSTM on the controllable Memory-Maze diagnostic---an intermediate gate; WP3 then carries the integration to interactive ARC-AGI-3 environments. Orchestration ablations must show the primary NCA/message-passing design outperforms at least two alternatives.

\subsubsection*{WP3: Long-Horizon Adaptation in Open-Ended Environments \textnormal{(Lepperød, PhD2; RQ4--RQ5; Years 3--5)}}

WP3 brings all three routes together---encode, retrieve, act---in a closed perception--action loop, stress-testing PAM in environments demanding sophisticated planning, resource management, and adaptation. We adopt a tiered ambition structure:
\begin{itemize}[noitemsep,topsep=2pt]
  \item \textbf{Must achieve (diagnostic):} $>$10\,pp over a parameter-matched LSTM on a controllable Memory-Maze diagnostic (memory demand decoupled from corridor length) at $T{=}10{,}000$.
  \item \textbf{Integration target:} \textbf{ARC-AGI-3} (ARC Prize Foundation, 2026)---interactive grid environments with no instructions (current LLMs $<$8\%, humans 100\%), the fusion point of PAM's episodic and program memory, evaluated on sample-efficiency and cross-level transfer.
  \item \textbf{Stretch goal:} open-ended crafting and planning---simplified Minecraft via the Malmo API~\cite{johnson2016malmo} and Factorio production chains~\cite{kant2025factorio}---across $T{>}10{,}000$ steps.
\end{itemize}
...
\subsection{Work and time schedule}\label{sec:schedule}
...
\noindent\textbf{Estimated compute:} WP1: $\sim$20K GPU-hours; WP2: $\sim$30K GPU-hours (informed by PAMNCA's 15K hours for preliminary work); WP3: $\sim$80K GPU-hours ($\sim$800 GPU-hours per DreamerV3-scale training run $\times$ $\sim$100 experiments across curriculum stages and ablations). Total: $\sim$130K GPU-hours. Existing Simula eX3 and national Sigma2 allocations cover the WP1--2 development phase ($\sim$50K hours); the WP3 scale-up ($\sim$80K hours) exceeds these internal allocations and is what the additional funding request for compute covers.

\noindent\textbf{Team allocation:} Lepperød (PI, 50\%) provides scientific leadership across all WPs and leads WP2. The Postdoc (PD, 100\%) joins in Year~1, onboarding on WP1 infrastructure and benchmarks before WP2 tasks begin, then co-leads WP2 with Lepperød from Year~2, contributing to T2.1--T2.4. PhD1 (100\%) starts Year~1 and leads WP1 (T1.1--T1.3). PhD2 (100\%) starts Year~2 and leads WP3 (T3.1--T3.3). An additional PhD student may be hired in Year~3 depending on progress.
...
Scientific & Modular architecture fails to specialize. & Med & High & Progressive training with soft inductive biases; Gumbel-softmax entropy floor prevents module collapse. \\
 & Encoder drift degrades content-addressable retrieval (stored keys go stale as representations learn online). & Med & High & Key reconsolidation (T1.3); hybrid fast/slow memory with Bayesian consolidation; WTA write prevents slot mean-convergence. \\
 & From-scratch (tPC) encoder fails to learn representations online (collapse already observed and diagnosed). & Med & High & Reconstruction/contrastive anchoring + streaming readout; frozen self-supervised features as the demonstrated fallback. \\
 & Compositional \textit{inference} does not follow from compositional representation (M2 gate: 29\% vs 80\% bar). & Med & High & Iterative infer-apply-residual (resonator) encoding; per-episode TTT, which already works for PAMNCA. \\
Technical & Computational resources exceed expectations. & Med & Med & Sigma2 + eX3 allocations plus additional compute funding (Section~\ref{sec:schedule}); model distillation for efficiency. \\
...
Field & LLM-based agents partially achieve target capabilities during the project. & Med & Med & Value proposition shifts to efficiency, formal memory guarantees, and scientific understanding of memory; success criteria are efficiency-qualified (parameters, compute). \\
\bottomrule
\end{tabularx}
}

\noindent\textbf{Feasibility:} The preliminary results demonstrate that the memory core works---92.5\% class-incremental accuracy on CORe50-NI (above a gradient-trained replay head) and 40\% ARC leave-one-out at 1.47M parameters. Equally important, a post-fix engineering review (gradient-flow audit, regression tests guarding every training path) makes these numbers trustworthy and the remaining risks \textit{diagnosed rather than unknown}: the from-scratch encoder collapse (WP1) and the compositional-inference gap (WP2) are open problems with concrete mechanisms and escalation paths, not surprises. Each WP has independent value: even if WP3's most ambitious targets are not reached, WP1--2 produce publishable architectures and benchmarks. The structured ablation ladder in WP2 ensures at least one orchestration mechanism succeeds, and the 4-tier hierarchy provides natural checkpoints before investing in WP3 environments.
...
\begin{itemize}[noitemsep,topsep=2pt]
  \item \textbf{Compute infrastructure:} Access to Simula's eX3 HPC cluster (A100/H100 GPUs) and national Sigma2 (Saga, Olivia) resources, which cover the WP1--2 development phase; see the per-WP compute breakdown in Section~\ref{sec:schedule} for the WP3 scale-up funded by the additional compute request.
...
RCN (SKI) & $\sim$200 MNOK & SURE-AI national AI centre (WP co-lead, Simula host) & Funded 2025--33 \\
RCN & 9 MNOK & SPARK: efficient spiking-network training (WP co-lead) & Funded \\
RCN IKTPLUSS & 12 MNOK & PASSION: AI safety and alignment & Not funded \\
