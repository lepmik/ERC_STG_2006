...
\newpage
% Visible red TODO markers for facts only the PI can supply — remove before submission.
\newcommand{\pitodo}[1]{\textcolor{red}{[TODO: #1]}}
...
\begin{enumerate}[noitemsep,topsep=2pt]
  \item \textbf{Lepperød ME} et al.\ \textit{Optogenetic pacing of medial septum parvalbumin-positive cells disrupts temporal but not spatial firing in grid cells}. Science Advances, 2021.
  ...
  \item \textbf{Lepperød ME} et al.\ \textit{Inferring causal connectivity from pairwise recordings and optogenetics}. PLoS Computational Biology, 2022. (With K.\ Kording.)
  ...
  \item Schøyen VS$^*$, Pettersen MB$^*$, \ldots, \textbf{Lepperød ME}. \textit{Coherently remapping toroidal cells but not grid cells are responsible for path integration in virtual agents}. iScience, 2023.
  ...
  \item Pettersen MB, Schøyen VS, Malthe-Sørenssen A, \textbf{Lepperød ME}. \textit{Decoding the cognitive map: Learning place cells and remapping}. eLife, 2024.
  ...
  \item Schøyen VS et al.\ \textit{Hexagons all the way down: Grid cells as a conformal isometric map of space}. PLOS Computational Biology, 2025.
...
\noindent\textbf{PAM / PAMNCA codebases:} Large-scale ML engineering behind the preliminary results in this proposal: systematic GPU-cluster experimentation ($\sim$15K GPU-hours on A40s), automated result parsing from 150+ log files, reproducible evaluation pipelines, and benchmarking across 5 real-world domains (CORe50, MIT-BIH, ESC-50, VoxCeleb, embodied navigation) using official online continual-learning protocols. \pitodo{repository URL and open-source release status}

\noindent\textbf{Community research infrastructure:} Co-developed NEST simulator modules within the Human Brain Project; co-developed the Exdir hierarchical data format and the Expipe experiment-management platform for reproducible neuroscience pipelines. \pitodo{adoption indicators: downloads/stars/citing labs for Exdir and Expipe}

\noindent\textbf{Benchmark contributions:} Evaluation on official public protocols (CORe50-NI, ARC). \pitodo{public leaderboard entries or benchmark submissions, if any}

\noindent\textbf{Team profile to complement the PI:} The FIND postdoc will be recruited from the ML systems community (target profile: PhD from an ICLR/ICML-publishing lab with large-scale training experience) to complement the PI's neuroscience background and own the WP2--3 scaling infrastructure. \pitodo{1--2 sentences on candidate pipeline, e.g.\ named ELLIS/NORA contacts}
